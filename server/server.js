...
app.post('/api/verify', (req, res) => {
  if (req.body.value) {
    request(
      `https://www.google.com/recaptcha/api/siteverify?secret="${key.capsec}"&response="${req.body.value}"`,
      (err, response, body) => {
...
app.post('/api/image/:type', (req, res) => {
  client
    ...
    .then(response => {
      let transformed = client.transform(response.handle, {
        output: { format: req.params.type },
      });
